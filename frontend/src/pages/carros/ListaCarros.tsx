import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { Button, Table, Alert, Pagination } from '../../components/common';
import { Header, Sidebar } from '../../components/layout';
import { api } from '../../services/api';
import type { Carro, ListaPaginada } from '../../types';
import { usePaginacao } from '../../hooks/usePaginacao';

export function ListaCarros() {
  const navigate = useNavigate();
  const pag = usePaginacao();
  const [data, setData] = useState<ListaPaginada<Carro>>();
  const [erro, setErro] = useState('');

  async function carregar() {
    setErro('');
    try {
      const res = await api.get(`/carros?page=${pag.page}&limit=${pag.limit}`);
      setData(res.data);
    } catch (err: any) { setErro(err.response?.data?.error || 'Erro'); }
  }

  useEffect(() => { carregar(); }, [pag.page, pag.limit]);

  async function excluir(id: number) {
    if (!window.confirm('Excluir carro?')) return;
    try {
      await api.delete(`/carros/${id}`);
      carregar();
    } catch (err: any) { setErro(err.response?.data?.error || 'Erro ao excluir'); }
  }

  const renderRow = (item: Carro) => (
    <tr key={item.id}>
      <td>{item.id}</td><td>{item.modelo}</td><td>{item.marca_nome}</td><td>{item.ano}</td>
      <td>R$ {item.preco}</td><td>{item.cor}</td>
      <td>
        <Button variant="secondary" onClick={() => navigate(`/carros/${item.id}/editar`)}>Editar</Button>{' '}
        <Button variant="danger" onClick={() => excluir(item.id)}>Excluir</Button>
      </td>
    </tr>
  );

  return (
    <div className="app-container">
      <Header />
      <div className="app-body">
        <Sidebar />
        <main className="app-main">
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
            <h2>Carros</h2>
            <Button onClick={() => navigate('/carros/novo')}>Novo Carro</Button>
          </div>
          <Alert type="error" message={erro} />
          <Table headers={['ID', 'Modelo', 'Marca', 'Ano', 'Preço', 'Cor', 'Ações']} data={data?.data || []} renderRow={renderRow} />
          <Pagination page={pag.page} totalPages={data?.paginacao?.totalPages || 1} onPageChange={pag.setPage} />
        </main>
      </div>
    </div>
  );
}
